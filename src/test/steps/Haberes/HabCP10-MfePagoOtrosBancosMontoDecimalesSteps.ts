...
import { MFEMedianteArchivo } from '../../../locators/MFE-MedianteArchivo';
  ...
  Given('DIEZ-HABERES-voy a Obe y iniciamos sesion',{timeout:80000}, async function () {
    await haberesPage.irAPage("10");
    await haberesPage.login();
  });

  When('DIEZ-HABERES-nos vamos a MFE Mediante Archivo, cargamos archivo con monto con decimales',{timeout:80000}, async function () {
    await haberesPage.MFEmedianteArchivo();
    await haberesPage.tipoLiquidacion('PAGO DE SUELDO');
  });

  When('DIEZ-HABERES-seleccionamos la fecha de pago y presionamos continuar',{timeout:80000}, async function () {
    await haberesPage.fechaPagoPosterior();
    await haberesPage.subirArchivo('C:/Users/Agnostic/Desktop/Test-Nominas-Haberes/Automatizacion/Pagodesueldos_OtrosBancosConDecimales.xlsx');
  
  });

  Then('DIEZ-HABERES-Nos envia a pantalla de confirmacion con los datos cargados en el archivo',{timeout:80000}, async function () {
    try{
        ...
        let monto = "//p[contains(text(),'$')]";
          ...
          await haberesPage.textExpect(monto, '$ 12.029,77');
