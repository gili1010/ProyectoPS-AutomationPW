...
import { MFEMedianteArchivo } from '../../../locators/MFE-MedianteArchivo';
  ...
  Given('CINCO-HABERES-voy a Obe y iniciamos sesion',{timeout:80000}, async function () {
    await haberesPage.irAPage("05");
    await haberesPage.login();
  });

  When('CINCO-HABERES-nos vamos a MFE Mediante Archivo, cargamos archivo con monto a pagar en cero',{timeout:80000}, async function () {    
    await haberesPage.MFEmedianteArchivo();
    await haberesPage.tipoLiquidacion('PAGO DE SUELDO');
  });

  When('CINCO-HABERES-seleccionamos la fecha de pago y presionamos continuar',{timeout:80000}, async function () {
    await haberesPage.fechaPagoPosterior();
    await haberesPage.subirArchivo('C:/Users/Agnostic/Desktop/Test-Nominas-Haberes/Automatizacion/Pagodesueldos-Supervielle-SaldoCero.xlsx');
  });

  Then('CINCO-HABERES-Nos envia a pantalla de error con alerta debe ingresar acreditaciones',{timeout:80000}, async function () {
    try{
